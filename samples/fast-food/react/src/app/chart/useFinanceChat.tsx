...
import { useChartTool } from './useChartTool';
...
interface FinanceChatState {
  ...
  isLoading: boolean;
  isEmpty: boolean;
  lastAssistantMessage: UiAssistantMessage<Chat.AnyTool> | undefined;
  sendMessage: (content: string) => void;
  resendMessages: () => void;
  stop: () => void;
}

const summarizeIngredients = (
  ingredients: Ingredient[],
): Omit<Ingredient, 'dailyReports'>[] => {
...
export const useFinanceChat = (): FinanceChatState => {
  const { chart, tool: toolJavaScript } = useChartTool();
  const [system] = useState(systemPrompt);

  const getIngredientsTool = useTool({
    name: 'getIngredients',
    ...
    handler: async (abortSignal) => {
      ...
      return summarizeIngredients(ingredients);
    },
    deps: [],
  });
  ...
  const {
    ...
    components: [
      exposeComponent(AiSuccess, {
        ...
        props: {
          title: AiSuccess.schema.title,
          message: AiSuccess.schema.message,
        },
      }),
      exposeComponent(AiRefusal, {
        ...
        props: {
          title: AiRefusal.schema.title,
          message: AiRefusal.schema.message,
        },
      }),
      exposeComponent(AiClarification, {
        name: 'app-ai-clarification',
        description: 'Show a clarification message to the user',
        props: {
          title: AiClarification.schema.title,
          message: AiClarification.schema.message,
        },
      }),
    ],
  });
  ...
  const code = useMemo(() => {
    for (let index = messages.length - 1; index >= 0; index -= 1) {
      ...
      const toolCall = message.toolCalls?.find(
        (call) => call.name === 'javascript',
      );
      if (!toolCall) continue;

      const args = toolCall.args as { code?: string } | undefined;
      if (args?.code) {
        return args.code;
      }
    }
  ...
  const isLoading = isSending || isReceiving || isRunningToolCalls;
