...
function fabricateDailyConsumption(seed: IngredientSeed, date: Date) {
...
function generateIngredient(
  ...
  let inventory = Math.round(seed.dailyConsumption.max * seed.leadTimeDays.max);
  ...
  return {
    ...
    safetyStock,
    reorderPoint,
    leadTimeDays: faker.number.int(seed.leadTimeDays),
...
function generateIngredientsDataset(options?: {
  startDate?: Date;
  numDays?: number;
  seed?: number;
}): Ingredient[] {
  ...
  const days = options?.numDays ?? SIM_DAYS;
  if (options?.seed !== undefined) {
    faker.seed(options.seed);
  }
  return INGREDIENT_SEEDS.map((seed) => generateIngredient(seed, start, days));
}

const seed = 424242; // deterministic output across builds
const data = generateIngredientsDataset({ seed });

writeFileSync(outFile, JSON.stringify(data, null, 2) + '\n', 'utf8');
console.log(`Wrote ${data.length} ingredients to ${outFile}`);
